...
def check_other_nicknames(clientsocket, nickname, address):
    with client_lock:
        current_client = clients.get(address)
        for addr, client_info in clients.items():
            if client_info.nickname == nickname:
                if addr == address:  # Check if the same client is trying to use the same nickname
...
#            if nickname == client_info.nickname:
...
def join_channel(clientsocket, address, channel_name):
    ...
    if channel.add_member(clients[address]):
        ...
        welcome_channel_message(clientsocket, channel_name)
...
def welcome_channel_message(clientsocket, channel_name):

    welcome_message = f"Welcome to {channel_name}"

    clientsocket.sendall(f":{socket.gethostname()} JOIN {channel_name} :{welcome_message}\r\n".encode('utf-8'))
